...
describe('ImagesService', () => {
  ...
  describe('getImagesByGallery', () => {
    it('should fetch images successfully', async () => {
      ...
      expect(mockedApi.get).toHaveBeenCalledWith(
        `galleries/${galleryId}/image?page=1&limit=20&order=asc`
      );
  ...
  describe('deleteImages', () => {
    ...
    it('should throw error if deletion fails', async () => {
      ...
      await expect(imageService.deleteImages(galleryId, payload)).rejects.toThrow('Delete Failed');
    });
  });
  ...
  describe('moveImages', () => {
    it('should move images successfully', async () => {
      ...
      const result = await imageService.moveImages(galleryId, targetGalleryId, payload);
    ...
    it('should throw error if move fails', async () => {
      ...
      await expect(imageService.moveImages(galleryId, targetGalleryId, payload)).rejects.toThrow(
        'Move Failed'
      );
    });
  });
  ...
  describe('copyImages', () => {
    it('should copy images successfully', async () => {
      ...
      const result = await imageService.copyImages(galleryId, targetGalleryId, payload);
    ...
    it('should throw error if copy fails', async () => {
      ...
      await expect(imageService.copyImages(galleryId, targetGalleryId, payload)).rejects.toThrow(
        'Copy Failed'
      );
    });
  });
